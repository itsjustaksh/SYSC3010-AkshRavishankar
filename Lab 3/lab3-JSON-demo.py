...
wind = results['wind']
for x in weather:
  cursor.execute('''INSERT INTO TABLE weather(temperature,humidity,pressure,wind) VALUES (%d,%d,%d,%d) ''',x['temp'],x['humidity'],x['pressure'],wind['speed'])

#print database
conn.execute('SELECT * FROM security')
for row in conn:
    print(row['id'],row['temperature'],row['city'])
